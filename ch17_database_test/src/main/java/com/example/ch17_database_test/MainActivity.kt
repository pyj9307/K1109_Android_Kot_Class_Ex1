...
class MainActivity : AppCompatActivity() {
    // 여기의 영역은 최고 상위 영역이라서 액티비티 한에
    // 다른 함수에서 접근하기 용이함
    // 주의 할 것은 기본은 선언하고 초기화 같이 해야되고
    ...
    // onCreate, restoreInstandeState, 3개만 매개변수로 번들 객체를 이용함.
    // 임시로 저장하면, 액티비티 화면에서 작업했던 에디트 텍스트 뷰라든지
    // 화면 회전을 하면 사라지는데, 번들객체에 자동으로 임시 저장 가능.
    // 단점 앱을 다시 실행하면 사라짐 -> 내부 및 외부 저장소를 이용함
    // 현재 SQLite 데이터베이스를 이용
    // 파일 직접 java,io 관련 스트림, 문자열 읽기, 쓰기 작업
    // 공유 프리퍼런스를 이용해서도 가능함. 임시로 파일 저장. 이게 현재 트렌드 작업을 많이 함.
    // 파이어 베이스로 연동 부분 설명 -> 화요일 시럼 -> 프로젝트 관련 설명
