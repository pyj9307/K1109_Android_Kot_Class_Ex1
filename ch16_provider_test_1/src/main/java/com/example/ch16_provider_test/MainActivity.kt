...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        {
            // contentResolver 외부에 저장소에 접근시 권한이 필요없음(권한이 없어도 갤러리에 접근가능)
...
//            var inputStream = contentResolver.openInputStream(it.data!!.data!!)
            try {
                ...
                // 조건, 파일에 쓰기 작업을 아랫쪽에 함수로 뺐음. 근데 fileUpload를 종료해버리는 중...
                if (inputStream != null) {
                    fileUpload(inputStream)
                }
                // 1) 현재, 액티비이에서 파일을 선언
                // 2) fileUpload 함수의 정의 부분에서 매개변수1: 사진첩에서 읽은 바이트 단위의 배열
                // 3) 두 매개변수 만들어서 : 파일로 쓰기위한 실제 물리 파일
                // 4) 실제 물리 파일에 사진의 이미지 저장.
                // 5) 실제 물리 파일의 경로를 알수 있음.
                // 6) 해당 경로의 Uri를 알아내서
                // 7) inputStream에 담을 예정
                // 8) bitmap 변환작업
                // 9) 붙이는 작업.



                // 이걸로 기존 사진첩에서 읽었던 바이트를 bitmap 변환해서, 해당 뷰에 붙이는 작업.
                var inputStream2 = contentResolver.openInputStream(it.data!!.data!!)

                // 외부 저장소 위치에 접근을 해서, inputStream으로 읽어야함.
    ...
    private fun fileUpload(inputStream: InputStream){
        ...
        try {
            ...
            // buffer 바이트 단위의 배열이고 지정한 크기만큼으로해서 전달용.
            val buff = ByteArray(1024 * 4)
            // os -> file : 실제 저장이 될 파일의 위치, 해당 파일에 바이트로 파일에 쓰기 작업. 아래 while문
